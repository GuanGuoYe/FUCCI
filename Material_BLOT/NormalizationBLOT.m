...
if  isnumeric(CellName{k0,1}{k1,k2})==0
    ...
    load('Storage\WorkSpace_DataGreen')      ; load('Storage\WorkSpace_DataRed') ;
for k0=1:length(DataGreen)
for k1=1:size(DataGreen{k0,1},1)
for k2=1:size(DataGreen{k0,1},2)
if  isempty(DataGreen{k0,1}{k1,k2})==0
    ZERO=find(DataGreen{k0,1}{k1,k2}(2,:)<0) ;
    if  isempty(ZERO)==0
        DataGreen{k0,1}{k1,k2}(2,ZERO)=0     ;
    end
end
end
end
...
if  isempty(DataRed{k0,1}{k1,k2})==0
    ZERO=find(DataRed{k0,1}{k1,k2}(2,:)<0)   ;
    if  isempty(ZERO)==0
        DataRed{k0,1}{k1,k2}(2,ZERO)=0       ;
    end
end
end
end
...
for k1=1:size(DataGreen{k0,1},1)
for k2=1:size(DataGreen{k0,1},2)
if  isempty(DataGreen{k0,1}{k1,k2})==0 && isempty(find(DataGreen{k0,1}{k1,k2}(2,:)<0))==0
    CellName{k0,1}{k1,k2}
end
end
end
...
for k1=1:size(DataRed{k0,1},1)
for k2=1:size(DataRed{k0,1},2)
if  isempty(DataRed{k0,1}{k1,k2})==0 && isempty(find(DataRed{k0,1}{k1,k2}(2,:)<0))==0
    CellName{k0,1}{k1,k2}
end
end
end
end
    save('Storage\WorkSpace_DataGreen','DataGreen','-v7.3') ;
    save('Storage\WorkSpace_DataRed'  ,'DataRed'  ,'-v7.3') ;
...
for I=1:length(Lineage)-1
    ...
    plot((dataGreen{1,I}(1,end)+dataGreen{1,I+1}(1,1))/2*[1,1],[-800,14000],'k--','linewidth',1) ; hold on               ;
end
    axis([-10,350,-1000,14000]) ; x=xlabel('\rmDevelopmental Time (min)') ; set(x,'Fontname','arial','Fontsize',17.5)    ;
...
if  isnumeric(CellName{k0,1}{k1,k2})==0 && strcmp(NameGreen0{I,1},CellName{k0,1}{k1,k2})==1
    ...
    DATA=cell2table(DATA)   ; writetable(DATA,'Storage\Table S_CYB-1_Blot.csv')  ;

    load('Storage\WorkSpace_DataRed_PLUSPLUSPLUSPLUS') ;
    tpGreen=250 ; tpRed=240 ; resolutionGreen=84.5/60  ; resolutionRed=84.5/60   ; % Parameter Setting
    Data=importdata('CDT-1_RED.csv')                                             ; % Path of CYB-1 File
    N=Data.data ; N=N(:,2)  ; C=Data.textdata ; C=C(2:size(C,1),1) ; NameRed={}  ; NameRed0={} ; DATA={} ;
for I=1:length(C)
    I/length(C)
...
if  isnumeric(CellName{k0,1}{k1,k2})==0 && strcmp(NameRed0{I,1},CellName{k0,1}{k1,k2})==1
    ...
    DATA=cell2table(DATA)   ; writetable(DATA,'Storage\Table S_CDT-1_Blot.csv')       ;